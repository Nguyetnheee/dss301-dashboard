...
export const demoRuleConfig: RuleConfig = {
  battery_critical_pct: 15,
  battery_caution_pct: 30,
  power_high_w_train_q80: 170,
  wind_caution_mps_train_q90: 8,
  distance_far_m_train_q75: 1000,
  status: 'Demo prototype assumptions',
};
...
export function applyDecisionRule(record: TelemetryRecord, predictedPower: number, config: RuleConfig): Pick<QueueRecord, 'recommended_action' | 'decision_reason'> {
  ...
  if (record.battery_level_pct <= config.battery_caution_pct && record.distance_to_base_m >= config.distance_far_m_train_q75 && predictedPower >= config.power_high_w_train_q80) {
    return { recommended_action: 'Return to Base', decision_reason: 'Battery is in the caution band while distance and predicted power demand are high relative to this dataset.' };
  }
